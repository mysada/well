...
class CartItemController extends Controller
{
    ...
    /**
     * Fetch cart items for the logged-in user (Created by Manish to Fetch Cart
     * Summary in Checkout).
     *
    ...
    public function store(Request $request)
    {
        ...
        if ($cartItem) {
            ...
            $cartItem->quantity
                                = $request->quantity; // Update with the submitted quantity
            $cartItem->subtotal = $product->price * $cartItem->quantity;
            $cartItem->total
                                = $cartItem->subtotal; // Assuming total is the same as subtotal
            $cartItem->items
                                = $cartItem->quantity; // Set items equal to quantity
            $cartItem->save();
        } else {
            ...
            CartItem::create([
              ...
              // Use the submitted quantity
              'items'      => $request->quantity,
              // Set initial items count to the submitted quantity
              'subtotal'   => $product->price * $request->quantity,
              'total'      => $product->price * $request->quantity,
            ]);
        }

        return redirect()->route('CartIndex')->with(
          'success',
        ...
        if ($cartItem) {
            ...
            return response()->json([
              'message'   => $request->quantity == 0
                ? 'Removed item successfully' : 'Updated successfully',
              'itemCount' => $itemCount,
    ...
    {
        if ($id) {
            return redirect(url('/cart_items'))->with(
              ['success' => $msg, 'id' => $id]
            );
        }
