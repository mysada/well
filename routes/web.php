...
Route::get(
  ...
  [CancellationRefundsController::class, 'index']
)->name('cancellation_refunds');
...
Route::post('/products/{id}/reviews', [ReviewController::class, 'store'])->name(
  'reviews.store1'
);
...
Route::middleware('auth')->group(function () {
    ...
    Route::get('/thank-you/{orderId}', [ThankYouController::class, 'show'])
         ->name('thankyou');
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::get('/checkout/{id}', [CheckoutController::class, 'showCheckout'])
         ->name('checkout.show');
    Route::post('/checkout/process', [CheckoutController::class, 'process'])
         ->name('checkout.process');
    Route::post(
      '/profile/set-default-address/{orderId}',
      [UserController::class, 'setDefaultAddress']
    )->name('user.setDefaultAddress');
    Route::put(
      '/profile/update-address/{orderId}',
      [UserController::class, 'updateAddress']
    )->name('user.updateAddress');
    Route::put(
      '/profile/update-default-address',
      [UserController::class, 'updateDefaultAddress']
    )->name('user.updateDefaultAddress');
    Route::get(
      '/profile/set-default-address/{orderId}',
      [UserController::class, 'setDefaultAddress']
    )->name('user.setDefaultAddress');
    Route::post(
      '/profile/set-default-address',
      [UserController::class, 'setDefaultAddress']
    )->name('user.setDefaultAddress');
});
...
Route::group(['middleware' => 'auth'], function () {
    Route::get('/admin/queries', [ContactQueryController::class, 'index'])
         ->name('admin.queries');
});
