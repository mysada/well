...
Route::get('order/details/{orderId}', [OrderController::class, 'details'])
     ->name('order.details');
Route::get('thank-you/{orderId}', [ThankYouController::class, 'show'])->name(
  'thankyou'
);

// Wishlist operations
Route::resource('wishlists', WishlistController::class)->except(
...
// Checkout process
Route::get('checkout/{id}', [CheckoutController::class, 'showCheckout'])->name(
  'checkout.show'
);
Route::post('checkout/process', [CheckoutController::class, 'process'])->name(
  'checkout.process'
);
