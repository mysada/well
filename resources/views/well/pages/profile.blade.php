            ...
            <div class="profile-avatar">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                          d="M5.121 17.804A11.953 11.953 0 0112 15c2.347 0 4.548.676 6.443 1.837M16 11a4 4 0 11-8 0 4 4 0 018 0zM12 21c-4.418 0-8-3.582-8-8a8 8 0 1116 0c0 4.418-3.582 8-8 8z"/>
                </svg>
            </div>
            <div class="profile-info">
                <h2>Welcome, {{ $firstName }} {{ $lastName }}!</h2>
                <p>Thank you for being a valued customer. Here you can view your order history, update your addresses,
                    and manage your profile settings.</p>
            </div>
        </div>
                        ...
                        @foreach ($orders as $order)
                            <div class="order-item">
                                ...
                                <p><strong>Total Price:</strong>
                                    ${{ number_format($order->orderDetails->sum('price'), 2) }}</p>
                                ...
                                <div class="buttons-section mt-4">
                                    <button onclick="location.href='{{ route('order.reorder', $order->id) }}'">Reorder
                                    </button>
                                    <button id="order_detail"
                                            onclick="location.href='{{ route('order.details', $order->id) }}'">Order
                                        Details
                                    </button>
                                </div>
                            </div>
                ...
                <div id="saved-addresses" class="profile-section" style="display:none;">
                    ...
                    @foreach ($lastOrders as $index => $order)
                        @if ($order->payments->isNotEmpty())
                            ...
                            <div class="address-box">
                                ...
                                <p>{{ $payment->billing_name }}<br>{{ $payment->billing_address }}
                                    , {{ $payment->billing_city }}, {{ $payment->billing_province }}
                                    , {{ $payment->billing_country }} - {{ $payment->billing_postal_code }}
                                    <br>{{ $payment->billing_email }}<br>{{ $payment->billing_phone }}</p>
                                <h6>Shipping Address</h6>
                                <p>{{ $order->shipping_name }}<br>{{ $order->shipping_address }}
                                    , {{ $order->shipping_city }}, {{ $order->shipping_province }}
                                    , {{ $order->shipping_country }} - {{ $order->shipping_postal_code }}
                                    <br>{{ $order->shipping_email }}<br>{{ $order->shipping_phone }}</p>
                            </div>
                        @endif
                    @endforeach
                    ...
                    @if ($defaultAddress)
                        <div class="address-box">
                            ...
                            <p>{{ $defaultAddress->shipping_name }}<br>{{ $defaultAddress->shipping_address }}
                                , {{ $defaultAddress->shipping_city }}, {{ $defaultAddress->shipping_province }}
                                , {{ $defaultAddress->shipping_country }} - {{ $defaultAddress->shipping_postal_code }}
                                <br>{{ $defaultAddress->shipping_email }}<br>{{ $defaultAddress->shipping_phone }}</p>
                            <h6>Default Billing Address</h6>
                            <p>{{ $defaultAddress->billing_name }}<br>{{ $defaultAddress->billing_address }}
                                , {{ $defaultAddress->billing_city }}, {{ $defaultAddress->billing_province }}
                                , {{ $defaultAddress->billing_country }} - {{ $defaultAddress->billing_postal_code }}
                                <br>{{ $defaultAddress->billing_email }}<br>{{ $defaultAddress->billing_phone }}</p>
                        </div>
                    @else
                    ...
                    <form method="POST" action="{{ route('user.setDefaultAddress') }}">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="shipping_name" id="shipping_name"
                                   ...
                                   value="{{ old('shipping_name', $defaultAddress->shipping_name ?? $user->shipping_name) }}"
                                   required>
                            @error('shipping_name')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="shipping_address" id="shipping_address"
                                   ...
                                   value="{{ old('shipping_address', $defaultAddress->shipping_address ?? $user->shipping_address) }}"
                                   required>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="shipping_city" id="shipping_city"
                                   ...
                                   value="{{ old('shipping_city', $defaultAddress->shipping_city ?? $user->shipping_city) }}"
                                   required>
                            @error('shipping_city')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="shipping_province" id="shipping_province"
                                   ...
                                   value="{{ old('shipping_province', $defaultAddress->shipping_province ?? $user->shipping_province) }}"
                                   required>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="shipping_country" id="shipping_country"
                                   ...
                                   value="{{ old('shipping_country', $defaultAddress->shipping_country ?? $user->shipping_country) }}"
                                   required>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="shipping_postal_code" id="shipping_postal_code"
                                   ...
                                   value="{{ old('shipping_postal_code', $defaultAddress->shipping_postal_code ?? $user->shipping_postal_code) }}"
                                   required>
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" name="shipping_email" id="shipping_email"
                                   ...
                                   value="{{ old('shipping_email', $defaultAddress->shipping_email ?? $user->shipping_email) }}"
                                   required>
                        ...
                        <div class="form-group">
                            ...
                            <input type="tel" name="shipping_phone" id="shipping_phone"
                                   ...
                                   value="{{ old('shipping_phone', $defaultAddress->shipping_phone ?? $user->shipping_phone) }}"
                                   required>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="billing_name" id="billing_name"
                                   ...
                                   value="{{ old('billing_name', $defaultAddress->billing_name ?? $user->billing_name) }}"
                                   required>
                            @error('billing_name')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="billing_address" id="billing_address"
                                   ...
                                   value="{{ old('billing_address', $defaultAddress->billing_address ?? $user->billing_address) }}"
                                   required>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="billing_city" id="billing_city"
                                   ...
                                   value="{{ old('billing_city', $defaultAddress->billing_city ?? $user->billing_city) }}"
                                   required>
                            @error('billing_city')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="billing_province" id="billing_province"
                                   ...
                                   value="{{ old('billing_province', $defaultAddress->billing_province ?? $user->billing_province) }}"
                                   required>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="billing_country" id="billing_country"
                                   ...
                                   value="{{ old('billing_country', $defaultAddress->billing_country ?? $user->billing_country) }}"
                                   required>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="billing_postal_code" id="billing_postal_code"
                                   ...
                                   value="{{ old('billing_postal_code', $defaultAddress->billing_postal_code ?? $user->billing_postal_code) }}"
                                   required>
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" name="billing_email" id="billing_email"
                                   ...
                                   value="{{ old('billing_email', $defaultAddress->billing_email ?? $user->billing_email) }}"
                                   required>
                            @error('billing_email')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <input type="tel" name="billing_phone" id="billing_phone"
                                   ...
                                   value="{{ old('billing_phone', $defaultAddress->billing_phone ?? $user->billing_phone) }}"
                                   required>
                            @error('billing_phone')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                                    ...
                                    <div class="review-header">
                                        <div class="review-user-info d-flex align-items-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                                 fill="currentColor" width="50" height="50" class="avatar-svg">
                                                <path fill-rule="evenodd"
                                                      d="M12 2a5 5 0 100 10 5 5 0 000-10zM3.5 18a8.5 8.5 0 0117 0v2a1 1 0 01-1 1h-15a1 1 0 01-1-1v-2z"
                                                      clip-rule="evenodd"/>
                                            </svg>
                                            <div>
                                                <h5 class="user-name">{{ $user->full_name }}</h5>
                                                <p class="review-date">Posted
                                        ...
                                        <div class="review-rating">
                                            @for ($i = 0; $i < 5; $i++)
                                                <svg xmlns="http://www.w3.org/2000/svg"
                                                     fill="{{ $review->rating > $i ? '#f8ce0b' : '#ccc' }}"
                                    ...
                                    <div class="review-body">
                                        ...
                                        @if($review->image)
                                            <div class="review-image">
                                                <img src="{{ url( $review->image) }}" alt="Review Image"
                            ...
                            @foreach ($pendingReviews as $order)
                                @foreach ($order->orderDetails as $detail)
                                    <div class="pending-review-item">
                                        ...
                                        <img src="{{ asset($detail->product->image_url) }}" alt="Product Image"
                                             class="img-fluid" width="100px">
                                        <p><strong>Order Date:</strong> {{ $order->created_at->format('Y-m-d') }}</p>
                                        <p><strong>Price:</strong> ${{ number_format($detail->price, 2) }}</p>
                                        <a href="{{ route('reviews.store', $detail->product->id) }}" id="update_button"
